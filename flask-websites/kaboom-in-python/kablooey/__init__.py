...
"""
script += """suckit.on('updater', (data) => {{
  fetch(`/socketio?event=updater&data=${JSON.stringify(data)}`).then(function(r) {{
    r.text().then(function(t) {{
      if(t.includes('BODY:ERRCOL')) {{
        t = t.replace('BODY:ERRCOL', '')
        $(document.body).css('background-color', 'black')
...
socketevents = {'updater':lambda data: endstring}
keydownfunc = None
actions = []
headstring = ''
screenobjs = []
...
def add(item, x, y, outline=False, opacity=False, z=False, tags=[], comps=[]):
  ...
  def filterer(i):
    if i == Area() or i == Body() or i == ArrowMove() or i == LetterMove():
      return ''
    return i
  newcomps = list(filter(filterer, comps))
  newcomps = list(filter(('').__ne__, newcomps))
  for i in newcomps:
    setattr(theobj, i.objattr, type(i)(theobj))
  socket.emit('updater')
  endstring += endstr
  return theobj
#Screen objects
...
class ScreenObj:
  ...
  def add_styles_string(self, string):
    global endstring
    oldel = self.el
    soup = BeautifulSoup(self.el, 'html.parser')
    div = soup.select('div')[0]
    div_style = cssutils.parseStyle(div["style"])
    div_style[string.split(':')[0]] = string.split(':')[1]
    div["style"] = div_style.cssText
    self.el = str(div)
    socket.emit('updater')
    endstring = endstring.replace(oldel, self.el)
...
@app.route('/')
def hello_world():
  return render_template(filetouse, kaboom=endstringr()+'</div>', scrpt=script+'</script>', headstf=headstring)
...
def socketioroute():
  if request.method == 'GET':
    if str(request.query_string)[2:-1] != '':
      ...
      form = {}
  return socketevents[form['event']](form['data'])
#ROON
def runner():
